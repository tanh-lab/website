export interface Publication {
     ...
     * columns stay level.
     *
     * The pixel size travels with the file. Every cover is a different shape,
     * and the slot is only as tall as the CSS says, so without it the browser
     * has no ratio to reserve and the img is laid out twice.
     *
     * Regenerated by hand rather than in CI, so the build stays hermetic and
     * needs no poppler. 248 is the slot at its tallest, twice over, which is
     * all a 124px box asks for on a retina screen. Note that HAL prepends its
     * own cover sheet, so a paper fetched from there wants page 2 rather than
     * page 1:
     *
     *   pdftoppm -png -r 90 -f 1 -l 1 paper.pdf out
     *   python3 -c "from PIL import Image; im=Image.open('out-1.png'); \
     *     im.thumbnail((248,248)); im.save('public/covers/<name>.webp','WEBP',quality=82)"
     */
...
export const publications: Publication[] = [
    ...
    },
    {
        ...
        cover: { src: "/covers/anira.webp", width: 192, height: 248 }
    }
];
