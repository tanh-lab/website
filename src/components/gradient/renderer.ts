...
/**
 * Create a gradient shader renderer. Works against either an OffscreenCanvas
 * (worker path) or an HTMLCanvasElement (main-thread fallback for browsers
 * that don't expose `transferControlToOffscreen`, e.g. Firefox Android).
 *
 * `raf` is injected because `requestAnimationFrame` lives on `window` in the
 * main thread and on `DedicatedWorkerGlobalScope` in the worker.
 */
export function createRenderer(
  ...
  let lastWidthCss = config.width
  let lastHeightCss = config.height
  let lastDpr = config.pixelRatio
  let startTime = 0
  let lastFrameTime = 0
  let frameIntervalMs = 1000 / Math.max(1, config.fps)
  let visible = true
  let disposed = false
  ...
  function uploadNoise() {
    if (!gl || gl.isContextLost() || !noiseBitmap) return
  ...
  function render(now: number) {
    if (disposed) return
    raf(render)
    if (!visible || !gl || !program || gl.isContextLost()) return
  ...
  ;(async () => {
    try {
      const res = await fetch(config.noiseUrl)
  ...
  return {
    resize(cssWidth, cssHeight, dpr) {
      lastWidthCss = cssWidth
      lastHeightCss = cssHeight
      lastDpr = dpr
      applySize(cssWidth, cssHeight, dpr)
    },
    setVisible(v) {
      visible = v
    },
